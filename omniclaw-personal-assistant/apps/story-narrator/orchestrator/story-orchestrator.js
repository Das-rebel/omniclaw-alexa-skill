...
const { withTimeout } = require(path.join(__dirname, '..', 'shared', 'resilience', 'timeout-wrapper'));
const { retryWithBackoff } = require(path.join(__dirname, '..', 'shared', 'resilience', 'retry'));
...
async generateStory(storyConfig) {
  ...
  /**
   * Auto-generate characters and story from just a theme/description
   * Characters are generated on-the-fly and automatically cleaned up after story ends
   * @param {Object} config - Story configuration (theme, setting, plotOutline only)
   * @returns {Promise<Object>} - Generated story with auto-generated characters
   */
  async autoGenerateStory(config) {
    const { theme, setting, plotOutline, interactive = false } = config;

    console.log(`[StoryOrchestrator] Auto-generating story: ${theme}`);

    try {
      const prompt = `Based on this story theme, generate 3-4 character profiles needed:

Theme: ${theme}
${setting ? `Setting: ${setting}` : ''}
${plotOutline ? `Plot: ${plotOutline}` : ''}

Generate characters in JSON format with this structure:
{
  "characters": [
    {
      "name": "Character Name",
      ...
      "speechPattern": "how they speak (pace, pitch, accent)",
      "emotionalRange": "emotional expressiveness",
      "quirks": ["unique speech habits"],
...
Respond ONLY with valid JSON, no other text.`;
      ...
      const charsJson = await this._generateWithRetry(prompt);
      const charsData = JSON.parse(charsJson);

      console.log(`[StoryOrchestrator] Auto-generated ${charsData.characters.length} characters`);
      ...
      for (const char of charsData.characters) {
        ...
        console.log(`[StoryOrchestrator] Created character: ${char.name}`);
      }

      const charList = Array.from(this.characters.keys());

      const storyPrompt = this._buildStoryPrompt({
        theme,
        characters: this._getCharacterDescriptions(),
        setting,
        plotOutline,
      ...
      const storyContent = await this._generateWithRetry(storyPrompt);
      const segments = this._parseStorySegments(storyContent);
      ...
      console.log(`[StoryOrchestrator] Auto-generated story with ${segments.length} segments`);

      return this.currentStory;

    } catch (error) {
      console.error('[StoryOrchestrator] Auto-generation failed:', error);
      throw new Error(`Failed to auto-generate story: ${error.message}`);
    }
  }

  /**
   * Clean up auto-generated characters after story playback
   * Call this after story is complete to free memory
   */
  cleanupAutoGenerated() {
    if (!this.currentStory?.autoGenerated) return;

    console.log('[StoryOrchestrator] Cleaning up auto-generated characters...');

    for (const [key, char] of this.characters) {
      if (char.isAutoGenerated) {
        this.characters.delete(key);
        console.log(`[StoryOrchestrator] Removed: ${char.name}`);
      }
    }

    this.currentStory = null;
    console.log('[StoryOrchestrator] Cleanup complete');
  }
...
Keep segments 15-25 words for optimal TTS performance.`;
  ...
  async _generateWithRetry(prompt) {
    const providers = [
      { name: 'cerebras', model: 'qwen-3-235b-a22b-instruct-2507', baseURL: 'https://api.cerebras.net/v1' },
      { name: 'groq', model: 'llama-3.3-70b-versatile', baseURL: 'https://api.groq.com/openai/v1' },
      { name: 'minimax', model: 'MiniMax-M2.7', baseURL: 'https://api.minimax.io/v1' }
    ];
    ...
    for (const provider of providers) {
      try {
        const providerClient = new (require('openai'))({
          apiKey: this._getApiKey(provider.name),
        ...
        const response = await withTimeout(
          ...
          25000,
          `${provider.name} story generation`
        );

        return response.choices[0].message.content;
      } catch (error) {
        console.error(`[StoryOrchestrator] ${provider.name} failed: ${error.message}`);
        lastError = error;
      }
    }
